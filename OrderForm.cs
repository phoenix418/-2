...
namespace ресторан_проект
{
    public partial class OrderForm : Form
    {
        ...
        {
            listBoxOrders.Items.Clear(); // Очищаем список     
            if (OrderService.UserOrders.ContainsKey(currentUser.Id))
            {
                foreach (var order in OrderService.UserOrders[currentUser.Id])
                {
                    listBoxOrders.Items.Add($"{order.Id}: {order.Description}");
                }
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (listBoxOrders.SelectedIndex >= 0) // Если заказ выбран
            {
                ...
                OrderService.DeleteOrder(currentUser.Id, selectedOrder.Id);
