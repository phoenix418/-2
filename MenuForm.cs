...
namespace ресторан_проект
{
    ...
    {
        private readonly User currentUser; // Текущий авторизованный пользователь
        private readonly List<MenuItem> menuItems; // Список блюд в меню
        private readonly List<MenuItem> cartItems = new List<MenuItem>(); // Корзина пользователя

        public MenuForm(User user)
        {
            InitializeComponent();
            currentUser = user;
            menuItems = MenuService.GetMenuItems(); // Загрузка меню из сервиса
            InitializeMenu();
        }
        ...
        // Добавление выбранного блюда в корзину
        ...
        // Удаление выбранного блюда из корзины
        private void buttonRemoveFromCart_Click_1(object sender, EventArgs e)
        {
            ...
            // Парсинг строки для получения имени и количества
        ...
        private void buttonPlaceOrder_Click_1(object sender, EventArgs e)
        {
            ...
            // Группировка товаров для подсчета количества
        ...
        private void UpdateCartDisplay()
        {
            ...
            // Группировка товаров по ID для отображения количества
            var itemGroups = new Dictionary<int, CartItemGroup>();
            foreach (var item in cartItems)
            {
                if (itemGroups.ContainsKey(item.Id))
            ...
            // Добавление элементов в ListBox корзины
            foreach (var group in itemGroups.Values)
            {
        ...
        // Вспомогательный класс для группировки товаров в корзине
